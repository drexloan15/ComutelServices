...
export default function AdminPortalPage() {
  ...
  const resolvedCount = useMemo(
    () => (ticketsQuery.data ?? []).filter((ticket) => ticket.status === "RESOLVED").length,
    [ticketsQuery.data],
  );
  const urgentCount = useMemo(
    () => (ticketsQuery.data ?? []).filter((ticket) => ticket.priority === "URGENT").length,
    [ticketsQuery.data],
  );
  ...
  return (
    <section className="space-y-6">
      ...
      <article className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl border border-blue-100 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">Resueltos</p>
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {!isLoading && filteredTickets.length === 0 && <p className="text-slate-600">No hay tickets.</p>}
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {isLoading && <p className="text-slate-600">Cargando...</p>}
        {!isLoading && (usersQuery.data?.length ?? 0) === 0 && <p className="text-slate-600">No hay usuarios.</p>}
